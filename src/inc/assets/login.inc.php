...
function login($email, $password, $check = "false")
{
    ...
    $stmt = $conn->prepare("SELECT user_id, user_name, user_surname, user_nick, user_email, user_avatar, user_desc, user_avatar, user_background  FROM users WHERE user_email = ? AND user_password = ? ");
    ...
    if ($num > 0) {
        startSession($data[0]['user_id'], $data[0]['user_name'], $data[0]['user_surname'], $data[0]['user_nick'], $data[0]['user_email'], $data[0]['user_desc'], $data[0]['user_avatar'], $data[0]['user_background']);
