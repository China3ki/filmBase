        ...
        if ($this->type = "film") {
            switch ($content) {
                ...
                case "characters":
                    ...
                    break;
                case "critics":
                    $this->query = "SELECT user_name, user_surname, user_avatar, film_name, rate_comment, rate FROM rate_show INNER JOIN films ON fk_rate_film=film_id INNER JOIN users ON fk_rate_user=user_id WHERE fk_user_type = 3 AND fk_rate_film = ?";
                    break;
                case "users":
                    $this->query = "SELECT user_nick, rate, rate_comment, user_avatar FROM rate_show INNER JOIN films ON fk_rate_film = film_id INNER JOIN users ON fk_rate_user = user_id WHERE fk_user_type = 1 AND fk_rate_film = ?";
                    break;
                case "genres":
